"""Collection management for Amplifier Workbench.

Provides collection discovery and resource loading, wrapping amplifier-collections
with workbench-specific search paths.
"""
...
class CollectionManager:
    """Manages collections for the workbench.

    Provides workbench-specific search paths and wraps amplifier-collections
    ...
    2. User-specific workbench: ~/.amplifier-workbench/collections/
    ...
    def _build_search_paths(self, extra_paths: list[Path]) -> list[Path]:
        ...
        # Middle: User-specific workbench collections
        user_workbench = Path.home() / ".amplifier-workbench" / "collections"
        if user_workbench.exists():
            paths.append(user_workbench)
